...
export default function PayrollPage({ params }: { params: { month: string } }) {
  const { data: session } = useSession();
  const [employees, setEmployees] = useState<Employee[]>([]);
  const [selectedMonth, setSelectedMonth] = useState<string>(params.month);
  ...
  // Fetch employees for the selected month
  useEffect(() => {
    const fetchEmployees = async () => {
      try {
        const response = await fetch(`/api/finance/payroll/${selectedMonth}`);
        if (response.ok) {
          const data = await response.json();
          setEmployees(data.map((emp: Employee) => ({ ...emp, paymentStatus: 'pending' })));
        } else {
          toast.error('Failed to fetch employees for this payroll');
        }
      } catch (error) {
    ...
    try {
      ...
      const validEmployees = selectedEmployeeDetails.filter(emp => {
        if (paymentMethod === 'mpesa') {
          return emp.phone && emp.phone !== 'N/A' && emp.phone.length > 0;
        } else { // bank
  ...
  const eligibleEmployees = employees.filter(emp => {
    const matchesPaymentMethod = paymentMethod === 'bank'
      ...
      : emp.phone && emp.phone !== 'N/A' && emp.phone !== null && emp.phone.length > 0;
  ...
  return (
    <div className="container mx-auto py-10">
      <Card className="max-w-6xl mx-auto">
        <CardHeader>
          <CardTitle>Process Payroll for {new Date(selectedMonth).toLocaleString('default', { month: 'long', year: 'numeric' })}</CardTitle>
